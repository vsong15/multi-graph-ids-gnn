...
OUTPUT_FILE = "graphs/web_ids23_temporal_graph.pt"
RANDOM_STATE = 42
...
def compute_temporal_features(df, time_column):
    ...
    # Inter-arrival time per source IP: time since the previous flow from the same source
    df_temp = df[[time_column, "id.orig_h"]].copy()
    df_temp["_ts"] = ts
    df_temp["_prev_ts"] = df_temp.groupby("id.orig_h")["_ts"].shift(1)
    inter_arrival_src = (df_temp["_ts"] - df_temp["_prev_ts"]).fillna(0.0).to_numpy(dtype=np.float64)
    inter_arrival_src = np.clip(inter_arrival_src, 0.0, None)

    # Cyclical hour-of-day encoding (works for Unix timestamps; gracefully degrades otherwise)
    hour = (ts % 86400) / 3600.0
    hour_sin = np.sin(2.0 * np.pi * hour / 24.0)
    hour_cos = np.cos(2.0 * np.pi * hour / 24.0)

    return np.stack([time_norm, inter_arrival_src, hour_sin, hour_cos], axis=1).astype(np.float32)
...
def build_node_features(df, all_nodes, train_mask):
    ...
    node_features_df.columns = ["_".join(col) for col in node_features_df.columns]
...
edge_labels = torch.tensor(df["attack"].to_numpy(), dtype=torch.long)
...
# Temporal features appended to edge feature matrix
temporal_features = compute_temporal_features(df, time_column)
temporal_features = scale_features(temporal_features, train_mask)

edge_features_with_time = np.concatenate([edge_features, temporal_features], axis=1)
...
data = Data(
    ...
    edge_attr=torch.tensor(edge_features_with_time, dtype=torch.float),
...
data.edge_time = torch.tensor(df[time_column].fillna(0).to_numpy(), dtype=torch.float)

os.makedirs(os.path.dirname(OUTPUT_FILE), exist_ok=True)
torch.save(data, OUTPUT_FILE)

print("\nTemporal graph saved:")
print(OUTPUT_FILE)
...
print(f"Edge features: {data.edge_attr.shape[1]} ({len(EDGE_FEATURE_COLUMNS)} flow + 4 temporal)")
...
for name, mask in [("Train", data.train_mask), ("Val", data.val_mask), ("Test", data.test_mask)]:
    ...
    summary = {int(label.item()): int(count.item()) for label, count in zip(unique, counts)}
